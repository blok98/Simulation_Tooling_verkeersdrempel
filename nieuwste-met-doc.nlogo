...
to spawn_person
  ...
  ifelse speedbump_mode = false [set person_num 1] [set person_num 2]
  if ticks = [random_spawn] of turtle 0 [
    create-persons 1
    ask person person_num [
      set shape "cow"
      set color yellow
      set size 1.5
...
to go
  ...
  ask turtle 0[
    ...
    if any? speedbumps [
      let pos_speedbump [xcor] of speedbump 1
      ifelse pos_speedbump <= xcor + sight and pos_speedbump > xcor and speed > [speedborder] of speedbump 1[
        set break 0.00001][set break 0]
      ifelse pos_speedbump <= xcor - 0.2 and speed < speedlimit [
    ...
    if any? persons [
      let pos_person [xcor] of person person_num
      if pos_person <= xcor + sight [set break 0.00002 set acceleration 0]
      if xcor >= pos_person [ask person person_num [die] set deaths deaths + 1 set break 0 set acceleration 0.00001]
    ]
  ]
